...
import { Edit2, Save, X, CheckCircle, Lock, Eye, EyeOff, UserPlus, UserCheck, Users, FileText, MapPin, Shield } from 'lucide-react'
...
function Avatar({ name, size = 80 }: { name: string; size?: number }) {
  ...
  return (
    <div className="avatar font-bold" style={{ width: size, height: size, fontSize: size * 0.32, letterSpacing: '-0.02em' }}>
      {initials}
    </div>
  )
}
...
function PeopleModal({ title, userId, type, onClose }: {
  ...
  useState(() => {
    fetch(`/api/users/${userId}/${type}?limit=30`)
      .then(r => r.json())
      .then(data => {
        setPeople(data[type] ?? [])
        setLoading(false)
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-panel" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 400 }}>
          ...
          {people.map(person => (
            <div key={person.id} className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--border-soft)' }}>
              <Avatar name={person.full_name || 'Resident'} size={38} />
...
export default function ProfileClient({
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card mb-4 overflow-hidden">
        ...
        <div className="px-6 pb-5">
          ...
          <div className="flex items-end justify-between -mt-10 mb-4">
            <div className="ring-4 ring-white dark:ring-gray-900 rounded-full" style={{ background: 'var(--surface)' }}>
              <Avatar name={profile?.full_name || 'Me'} size={80} />
            </div>
          ...
          {!editing ? (
            ...
            <div className="space-y-3 mb-4">
              ...
              <div>
                ...
                <textarea className="input resize-none" rows={2} placeholder="Tell your neighbors a bit about yourself…"
      ...
      {/* Account details - only visible to own profile */}
      {isOwnProfile && (
        <>
          {/* Account info */}
          ...
          <div className="card p-5 mb-4">
            ...
            {showPwSection && (
              <div className="mt-4 pt-4 space-y-3" style={{ borderTop: '1px solid var(--border-soft)' }}>
                {pwError && <div className="p-3 rounded-xl text-sm text-red-700"
                  style={{ background: '#fee2e2', border: '1px solid #fca5a5' }}>{pwError}</div>}
                {pwSuccess && <div className="p-3 rounded-xl text-sm text-green-700"
                  style={{ background: '#dcfce7', border: '1px solid #86efac' }}>{pwSuccess}</div>}
                <div>
